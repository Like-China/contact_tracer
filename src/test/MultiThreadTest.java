...
class MultiThread implements Runnable
{
    ...
    public MultiThread(String name, String city_name, int duration_threshold, float distance_threshold, int sr, int initPatientNum, int objectNum)
    {
        threadName = name;
	...
	public void eta(HashSet<Integer> patientIDs)
    {
		...
		System.out.println("距离计算次数: " + etaTracer.D.calcCount);
    ...
    public void egp(HashSet<Integer> patientIDs, boolean prechecking)
    {
		...
		System.out.println("距离计算次数: " + tracer.D.calcCount);
...
public class MultiThreadTest {
    ...
    public static void main(String args[]) {
        ...
        //Beijing
		long t1 = System.currentTimeMillis();
    //    for(Integer patientNum:new int[]{200,400,600,800,1000})
    //    {
    //         new MultiThread(String.format("Beijing Thread-patientNum  %d", patientNum), "beijing", 15, 2f, 10, patientNum, 10000).start();
    //    }

    //    for(Integer duration:new int[]{5,10,20,25})
    //    {
    //         new MultiThread(String.format("Beijing Thread-duration  %d", duration), "beijing", duration, 2f, 10, 600, 10000).start();
    //    }

    //    for(Float distance:new float[]{4f,6f,8f,10f})
    //    {
    //         new MultiThread(String.format("Beijing Thread-distance %f", distance), "beijing", 15, distance, 10, 600, 10000).start();
    //    }
       ...
       {
            new MultiThread(String.format("Porto Thread-patientNum  %d", patientNum), "porto", 5, 2f, 5, patientNum, 1000000).start();
       }
       
       for(Integer duration:new int[]{5,7,9,11})
       {
            new MultiThread(String.format("Porto Thread-duration %d", duration), "porto", duration, 2f, 5, 60000, 1000000).start();
       }

       for(Float distance:new float[]{4f,6f,8f,10f})
       {
            new MultiThread(String.format("Porto Thread-distance %f", distance), "porto", 5, distance, 5, 60000, 1000000).start(); 
       }
