...
读取/home/Like/data/contact_tracer/taxi_porto_by_id文件夹，里面每个txt是一个taxi的轨迹
生成/home/Like/data/contact_tracer/porto_%d 指定采样率的轨迹文件，每个txt为一个月的轨迹
会生成很多个文件，多少天多少个文件
"""
...
class Interpolation:
    ...
    def __init__(self, read_txt_size, sr):
        ...
        # taxi 文本文件路径
        self.file_dir = os.path.join("/home/Like/data/contact_tracer/taxi_porto_by_id")
        # [经纬度坐标, 转化的时间戳]
        self.raw_trips = []
        # 插值后的所有轨迹
        self.all_pad_trips = []
        ...
        for trip in self.raw_trips:
            ...
            for ii in range(len(trip)-1):
                ...
                if cur_point[1][:11] != next_point[1][:11]:
                    date = cur_point[1][:10].split("-")
                    tail = ii+1
                    month_id = int(date[1])
                    all_days_trip[month_id].append(trip[head:tail])
                    head = tail
                # 记录最后一天，因为最后一天不能有区分
                if ii == len(trip)-2:
                    tail = ii+1
    ...
    def padding_all(self):
        ...
        for ii in range(1, 13):
            # 获取每个月的数据，按月存储
            day_trips = days_trips.get(ii)
            if len(days_trips) <= 10:
                continue
            self.all_pad_trips = []
            for trip in tqdm(day_trips, desc='轨迹插值至统一采样率'):
    ...
    def write_day_stream(self, day):
        ...
        dir_path = "/home/Like/data/contact_tracer/porto_%d" % self.sr
        if not os.path.exists(dir_path):
            os.mkdir(dir_path)
        f = open(dir_path+"/%d_%d.txt" % (day, len(self.all_pad_trips)), "w")
        for t_id, date, lon, lat, ts in tqdm(self.all_pad_trips, desc='正在写出轨迹流文件'):
